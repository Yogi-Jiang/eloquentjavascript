...
CanvasDisplay.prototype.updateViewport = function() {
    ...
    } else if (center.x + margin > view.left + view.width) {
        view.left = Math.min(center.x + margin - view. width,this.level.width - view.width);
    };

    if (center.y < view.top + margin) {
        view.top = Math.max(center.y - margin, 0);
    } else if (center.y + margin > view.top + view.height) {
...
CanvasDisplay.prototype.drawActors = function() {
    this.level.actors.forEach(function(actor) {
        ...
        if (actor.type == "player") {
            ...
            var tileX = actor.type == "coin"? 2 : 1;
            this.cx.drawImage(otherSprites,tileX * scale, 0, width, height,
                               x, y, width, height);
        }
    },this);
};
